...
func TestMergeChartImages(t *testing.T) {
	...
	// Write an initial values.yaml with standalone images and overrides.
	initial := `# Standalone images
overrides:
  timberio/vector:
    from: "distroless-libc"
    to: "debian"
...
`
	...
	if !strings.Contains(content, "library/redis") {
		t.Error("expected manual redis entry to be preserved")
	}
	if !strings.Contains(content, "overrides:") {
		t.Error("expected overrides section to be preserved")
	}

	// Chart-images section present.
	if !strings.Contains(content, "chart-images:") {
		t.Error("expected chart-images section")
	}
	if !strings.Contains(content, "prometheus/prometheus") {
		t.Error("expected prometheus image in chart-images")
	}
	if !strings.Contains(content, "prometheus/alertmanager") {
		t.Error("expected alertmanager image in chart-images")
	}

	// ParseImagesFile should find all images (manual + chart).
	images, err := ParseImagesFile(valuesPath)
	if err != nil {
		t.Fatalf("ParseImagesFile() error: %v", err)
	}
	if len(images) != 3 {
		t.Fatalf("expected 3 images (1 manual + 2 chart), got %d", len(images))
	}
}

func TestMergeChartImagesReplaces(t *testing.T) {
	dir := t.TempDir()
	valuesPath := filepath.Join(dir, "values.yaml")

	// Write initial with an existing chart-images section.
	initial := `redis:
  image:
    registry: docker.io
    repository: library/redis
...
# Images discovered from charts (auto-generated by verity discover, do not edit)
chart-images:
  old-image:
    image:
...
`
	...
	// Merge new chart images (should replace old section).
	newImages := []Image{
		{Registry: "quay.io", Repository: "new/image", Tag: "2.0.0"},
	}
	if err := MergeChartImages(valuesPath, newImages); err != nil {
		t.Fatalf("MergeChartImages() error: %v", err)
	}
	...
	// Old chart image should be gone.
	if strings.Contains(content, "old/image") {
		t.Error("expected old chart image to be replaced")
	}
	// New chart image should be present.
	if !strings.Contains(content, "new/image") {
		t.Error("expected new chart image to be present")
	}
...
`
	...
	// Merge empty list — should not add chart-images section.
	...
	if strings.Contains(string(data), "chart-images:") {
		t.Error("expected no chart-images section for empty input")
	}
}
